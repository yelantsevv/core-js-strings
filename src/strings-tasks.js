...
function sumOfCodes(str) {
  if (!str) return 0;
  return [...str].reduce((acc, x) => acc + x.charCodeAt(0), 0);
}
...
function isPalindrome(str) {
  const strPol = str.replace(/[^a-zA-Z]/g, '').toLowerCase();
  return strPol === [...strPol].reverse().join('');
}
...
function encodeToRot13(str) {
  ...
  const index = (x) => (base.indexOf(x) >= 0 ? res[base.indexOf(x)] : x);
  return [...str].map((x) => index(x)).join('');
}
